...
def item_inventory_tracking():
    ...
    while status == False:
        ...
        1.Total available quantity of all items sorted in asending order by Item Code.
        ...
        if choice == 2:
            ...
            print_transactions_result()
        elif choice == 5:
            status = True
            print("Go Bck Successful")

def print_transactions_result():
    results=find_transactions_range_dates()
    for i in results:
        print(
            f'Item Code: {i[0]} ,Supplier or Hospital COde: {i[1]} ,Transaction Type :{i[2]} , Quantity: {str(i[3])[:10]}, Date: {i[4]}')
...
def reminder():
    ...
    for ppe in ppe_rec:
        if int(ppe[3]) <= 25:
            print(f"The quantity of item {ppe[1]} is less than 25 boxes.")
...
def read_transactions():
    ...
    with open("transactions.txt", "r") as file:
        ...
        for line in lines:
            item_code, supplier_hospital, quantity, str_date, transaction = line.strip().split(",")
            date = datetime.datetime.strptime(str_date, "%d/%m/%Y")
            transactions_rec.append((item_code, supplier_hospital, quantity, date, transaction))
        return transactions_rec


def find_transactions_range_dates():
    results = []
    found = False
    transactions_rec = read_transactions()
    str_date_start = input("Please enter the starting date of tracking the item (Exp: 1/1/2023) : ")
    start_date = datetime.datetime.strptime(str_date_start, "%d/%m/%Y")
    str_date_end = input("Please enter the ending date of tracking the item (Exp: 1/1/2023) : ")
    end_date = datetime.datetime.strptime(str_date_end, "%d/%m/%Y")

    for rec in transactions_rec:
        if start_date <= rec[3] <= end_date:
            found = True
            results.append(rec)
...
def search_transactions(transaction_rec, skey):
    found = False
    for i in transaction_rec:
        if i[4] == skey:
            print("‖", i[0], " " * (10 - len(i[0])), "|", i[1], " " * (27 - len(i[1])), "|", i[2],
                  " " * (28 - len(i[2])), "|", str(i[3])[:10], " " * (10 - len(str(i[3])[:10])), "|")
            found = True

    if not found:
        print("No transactions found for the given search key.")

def search_functionalities():
    transaction_rec = read_transactions()
    skey = input("What type of transaction for any particular item you want to search? [Received(R)/Distribution(D)]: ")

    if skey.lower() == "d":
        print("The Transaction of Distribution item:")
        search_transactions(transaction_rec, "d")
    elif skey.lower() == "r":
        print("The Transaction of item Received:")
        search_transactions(transaction_rec, "r")
    else:
        print("Please enter a valid transaction type (R or D).")
